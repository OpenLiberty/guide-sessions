...
You'll learn how to build an application which requires to use session. Beside to containerize 
and run the application in a local Kubenetes cluster, you'll also learn the Open Liberty `sessionCache`
feature to persist HTTP sessions using JCache, so that the session data can be shared among pods 
