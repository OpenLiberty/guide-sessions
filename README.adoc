...
The [hotspot=copyDependencies]`copyDependencies` tag copies the specified third-party [hotspot=hazelcast]`hazelcast` dependency to a specified location. Multiple [hotspot=dependency]`dependency` parameters and [hotspot=dependencyGroup]`dependencyGroup` parameters can be added to the [hotspot=copyDependenciesConfig]`copyDependencies` configuration. The location parameter can be added to the copyDependencies or dependencyGroup configuration to override the default [hotspot=location]`location`, which is the lib/global folder of the target server. 
