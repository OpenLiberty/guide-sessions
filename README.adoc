...
through the `<group/>` configuration element in Hazelcast configuration file.
...
The cluster group [hotspot=group file=1]`CartCluster` is specified in the `hazelcast-config.xml`.
