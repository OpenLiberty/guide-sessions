...
The [hotspot=cloneID file=1]`clone ID` must be unique for each server and consists of alphanumeric characters. Notice the `cloneId` variable in the [hotspot=bootstrapProperties file=0]`<bootstrap.properties\>`, located in the [hotspot file=0]`pom.xml`. Refer to the [hotspot=Properties]`<Properties/>` to section manually declare the cloneID.  `cloneId`, and other properties, such as the port number; [hotspot=cloneId-property[] file=0]`cloneId` and the [hotspot=port-property]`port number`, respectivley. 
