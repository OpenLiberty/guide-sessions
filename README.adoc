...
To ensure the persistence in using Hazelcast as the JCache provider, we can manually define the clone ID that the HTTP server plug-in uses. When using the `Liberty` server, it is good practice to manually assign the `clone ID`, since the ID is regenerated if the server is started with the `--clean` option. 
...
The `clone ID` must be unique for each server and consists of alphanumeric characters. Notice the `cloneId` configured in the [hotspot=bootstrapProperties file=1]`<bootstrap.properties\>`, located in the [hotspot file=1]`pom.xml`. Update the `pom.xml` to parameters of your choosing. Refer to the [hotspot=Properties file=1]`<Properties/>` section to manually declare the `cloneId`, along with other properties, such as the port number; [hotspot=cloneId-property file=1]`cloneId` and the [hotspot=port-property file=1]`port number`, respectively. 

For instance, [hotspot=cloneId-property file=1]`cloneId` can be set to `s1`, and the [hotspot=port-property file=1]`port number` can be set to `9081`.
