...
[role='command']
```
docker build -t guide-sessions:1.0-SNAPSHOT .
```
...
Verify that the `guide-sessions:1.0-SNAPSHOT` image is listed among them, for example:
----
REPOSITORY        TAG             IMAGE ID   
guide-sessions    1.0-SNAPSHOT    d28d25834149
open-liberty      latest          b23e29fb15e3
----

== Deploying the containized application

Now that the containized application is built, deploy it using a Kubernetes resource definition,
which is provided in the `kubernetes.yaml` file at the start directory.
...
session-deployment-6bd97d9bf6-4ccds   1/1     Running   0          15s
session-deployment-645767664f-nbtd9   1/1     Running   0          15s
session-deployment-645767664f-nbtd9   1/1     Running   0          15s
----
