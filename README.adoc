...
Your Hazelcast implementation of JCache is a JAR file which resides in the location defined by
...
Navigate to the `start` directory and run the following commands:
[role='command']
```
mvn package
...
Verify that the Hazelcast cluster is running. Check the logs of your pod using the `kubectl logs` command:
