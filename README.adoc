...
:page-description: Learn how to cache HTTP session data that is shared among pods inside Kubernetes cluster.
:guide-author: Open Liberty
:page-tags: ['Java EE', 'Docker', 'Microprofile']
...
:page-seo-description: A tutorial on how to cache HTTP session data that is shared among pods inside Kubernetes cluster by using Hazelcast as the Java Caching (JCache) provider.
= Caching HTTP session data

Learn how to create and work with HTTP session data that is shared among pods inside Kubernetes cluster by using Java Caching (JCache) in Open Liberty.
...
such as https://www.docker.com/[Docker^], where it can be deployed to Kubernetes.

=== What is kubernetes?
Kubernetes is an open source container orchestrator that automates many tasks that are involved in deploying, 
...
When your application runs in the Kubernetes cluster environment, the application session data has to be in-sync among all instances. 
Even if an application instance is down, the other instances can work and handle the session from the same user connection.
...
To demonstrate how HTTP session data can be shared in multiple instances of the same application, 
this guide shows you how to build an application that uses session data, containerizes the application, and runs it in 
a local Kubernetes cluster. 
You'll also learn about the Open Liberty `sessionCache` feature to persist HTTP sessions using JCache, the standard caching API for Java,  and understand that the session data can be shared among pods inside the Kubernetes cluster.

// *Application Containerization* 
You will learn about application containerization because the session caching model that will be demonstrated focuses on connectivity between servers. 
Application containerization is to bundle an application with all of its required libraries,
dependencies, and configuration files, so that it can run in any containerization ecosystem, 
such as https://www.docker.com/[Docker^], where it can be deployed to Kubernetes.
// *Kubernetes* +
...
The application that you’ll be working with is a shopping cart web service which uses `jaxrs` which is a Java API for building RESTful web services.
...
are used by the `mpOpenAPI` and `jaxrs` features to provide context to
...
You'll use the peer-to-peer model for this guide with Kubernetes as this model is focused on connectivity between servers. 
...
You can limit the session cache being shared only among certain Hazelcast instances 
through the [hotspot=group file=1]`<group />` configuration tag in Hazelcast configuration file.
...
the [hotspot=httpSessionCache file=0]`httpSessionCache` tag. When the maven build is run,
...
endpoint on the UI and clicking the `Try it out` button.
...
== Deploying and running the containerized application
...
Run the following command to deploy the application into [hotspot=replicas file=0]`three` pods as defined 
in kubernetes.yaml:
[role='command']
```
...
Repeat the GET request. You'll see the `session-id` should be the same 
