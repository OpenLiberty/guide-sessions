...
through the `<group/>` configuration tag in Hazelcast configuration file.
...
The cluster group [hotspot=group file=1]`CartCluster` is defined in the `hazelcast-config.xml`.
