...
by using the [hotspot=group file=0]`<group />` configuration tag in the Hazelcast configuration file.
...
The cluster group [hotspot=cartCluster file=0]`CartCluster` is defined in the [hotspot file=0]`hazelcast-config.xml`.

[role='command']
include::{common-includes}/devmode-start.adoc[]

=== Copying Hazelcast configuration and JAR files to a shared directory

In the Maven `pom.xml` file, the `package` goal is defined to copy the 
[hotspot file=0]`hazelcast-config.xml` file that you previously created
and a Hazelcast JAR file to a shared directory that resides in the `target` directory.

Run the `mvn package` command from the `start` directory in another shell session:
...
When the Maven build runs, the [hotspot file=0]`hazelcast-config.xml` file is copied to the predefined `{shared.config.dir}` directory.
...
The [hotspot=sessionCache file=0]`sessionCache` feature uses JCache, which allows for session
...
Add the [hotspot=sessionCache file=0]`sessionCache` feature in the [hotspot file=0]`server.xml` file.
