...
The [hotspot=cloneID]`clone ID` must be unique for each server and consists of alphanumeric characters. You can modify the [hotspot=bootstrapProperties]`<bootstrap.properties\>` in the [hotspot file=2]`pom.xml` to add the required vairables in order to manually declare the cloneID. 
