...
:page-seo-description: Learn how to cache HTTP session data that is shared among pods inside Kubernetes cluster.
...
this guide will show you to build an application using session data, containerize, and run it in 
a local Kubernetes cluster. 
Youll also learn the Open Liberty `sessionCache` feature to persist HTTP sessions using JCache. 
Youll see the session data can be shared among pods inside the Kubernetes cluster.
...
Go to the `start` directory to begin.
...
The endpoint [hotspot=endpointCartItemPrice file=1]`cart/{item}&{price}` demonstrates how to set session data.
...
The endpoint [hotspot=endpointCart file=1]`cart` demonstrates how to get session data.
...
The [hotspot=library]`library` tag makes the server know where the Hazelcast implementation of
...
To limit the session cache being shared only among certain Hazelcast instances, you can 
set the `<group/>` in a Hazelcast configuration file.
...
The cluster group [hotspot=group file=2]`CartCluster` is specified in the `hazelcast-config.xml` file.

In the [hotspot file=1]`server.xml` file, a reference to the Hazelcast configuration file is made using
the [hotspot=httpSessionCache file=1]`httpSessionCache` tag. When the maven build is run,
the [hotspot file=2]`hazelcast-config.xml` file is copied to the predefined `{shared.resource.dir}` directory.
...
Run the following command to deploy the application into three pods as defined in kubernetes.yaml:
[role='command']
```
