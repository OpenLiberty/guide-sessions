...
Hazelcast JCache provides the client and member providers. Set `hazelcast.jcache.provider.type` to `member` for using the member provider.
...
First, use `ClusterRoleBinding` Kubernetes API object to grant Hazelcast members to access the cluster.
[role='command']
```
...
When you no longer need your deployed application, you can delete all {kube} resources and disable the Hazelcast members to access the cluster by running the `kubectl delete` commands:
