...
server.xml
[source, xml, linenums, indent=0, role="code_column hide_tags=copyright,cloneID"]
----
...
To ensure the persistence in using Hazelcast as the JCache provider, we can define the `clone ID`. When using the Open Liberty Liberty server, it is good practice to assign the `clone ID`, as the ID can change after the scaling of pods, since the ID is regenerated if there is a server restart, or the server is started with the `--clean` option. This will also ensure to alleviate sessions pile up in Hazelcast. 

To learn more about the clone ID configuration, please visit: https://www.ibm.com/support/knowledgecenter/en/SSEQTP_liberty/com.ibm.websphere.wlp.doc/ae/twlp_admin_session_persistence_jcache.html[Configuring Liberty session persistence with JCache^]
...
The `clone ID` must be unique for each server and consists of alphanumeric characters. Notice the `cloneId` configured in the [hotspot=bootstrapProperties file=1]`<bootstrap.properties\>`, located in the [hotspot file=1]`pom.xml`. Update the `pom.xml` to parameters of your choosing. Refer to the [hotspot=Properties file=1]`<Properties/>` section to declare the `cloneId`, along with other properties, such as the port number; [hotspot=cloneId-property file=1]`cloneId` and the [hotspot=port-property file=1]`port number`, respectively. 

For instance, [hotspot=cloneId-property file=1]`cloneId` can be set to `s2`, and the [hotspot=port-property file=1]`port number` can be set to `9081`.

This will ensure that the ID is stable and that the request affinity is maintained.
