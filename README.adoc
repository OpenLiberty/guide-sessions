...
predefined `{shared.resource.dir}` directory when the Maven build runs.
...
following Docker build command in the `start` directory. Run the `mvn package` command from the `start` directory 
so that the `.war` file resides in the `target` directory.
