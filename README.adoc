...
:page-seo-title: Caching and persisting session data in microservices
:page-seo-description: Learn how to create and cache session data that persists between servers.
:imagesdir: assets/
= Caching and persisting HTTP sessions in microservices
...
The microservice that you will be working with is a shopping cart web service. You'll
learn how to persist an user's shopping cart data between servers using
...
requests. Session variables store user information like usernames, items in a cart, etc.
...
You will create a microservice to store and retrieve session data.

Navigate to the `start` directory to begin.

[role="code_command", subs="quotes"]
----
#Create the `SessionApplication` class.#
`src/main/java/io/openliberty/guides/sessions/SessionApplication.java`
----
...
The [hotspot file=0]`SessionApplication` class extends the generic JAX-RS application class needed to run the
application.

[role="code_command", subs="quotes"]
----
#Create the `SessionResource` class.#
`src/main/java/io/openliberty/guides/sessions/SessionResource.java`
----
...
The [hotspot file=1]`SessionResource` class defines the REST endpoints at which a user can make
a HTTP request.
...
The [hotspot=28 hotspot=30 file=1]`@PathParam` annotation injects a custom [hotspot=28 file=1]`item` and [hotspot=30 file=1]`price` from the POST
...
The `sessionCache` feature is only valuable when it is connected to at least
...
You will use the peer-to-peer model for this guide.
...
This feature does not include a JCache implementation.
...
By default, all OpenLiberty servers running `sessionCache`, the [hotspot]`SessionApplication`
...
[role="code_command", subs="quotes"]
----
#Create a `SessionResourceTest` class.#
`src/test/java/it/io/openliberty/guides/sessions/SessionResourceTest.java`
----
