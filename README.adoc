...
Learn how to create and work with session data. Expand on this knowledge by implementing
...
This guide uses the MicroProfile OpenAPI.
...
Point your browser to the link:http://localhost:9080/openapi/ui/[] URL. This is
a user-interface representation of the available GET requests that can be made
on this server.

First make a GET request to `/sessions/session/addToCart/{item}&{price}`.
The addToCart call will add a user specified item and price to a session
that represents data in a users cart.

Make a second GET request to `/sessions/session/getFromCart`.  The getFromCart call
will get all the item and price data from this users session and display it as a string.

You can make these requests using the UI or by making direct requests by pointing
your browser to the locations below.

link:http://localhost:9080/LibertyProject/sessions/session/addToCart/SpaceShip&20.00[]
link:http://localhost:9080/LibertyProject/sessions/session/getFromCart[]

When you are ready to continue stop your server by running.

```
mvn install liberty:stop-server -DserverName=server1
```
...
Session persistence is a means to be able to handle client requests being routed to
different backends or to handle failover of the backend system.

High traffic websites must support thousands of users in a fast and reliable way.
When a user browses your website for ten minutes, they are probably going to request
information from multiple servers. This will cause problems if the backend server the
client started on (via load balancing) is keeping session data, but not communicating
that data.

Therefore, session persistence is used to ensure that users requesting multiple
resources can be completed seamlessly because of a server cache that allows
all servers to access session data.
...
The server.xml file runs when the server starts.  This file allows you to declare
what features you need for your application, and specify contextual information such
as port numbers and library references.
...
* A class that is defined as `@ApplicationScoped` will only be created once during
the duration of the application.  This ensures we are using the same application
between requests.
* The `@ApplicationPath` annotation identifies the application path, and serves as the
base URI.
...
The `sessionEndpoint` class defines the endpoints at which a user can make a HTTP
request.  This class builds on top of our application and defines paths for these
endpoints using the `@Path` annotation.

The `addToCart` method has a number of annotations:

* `@GET` indicates the type of HTTP request.
* `@Produces`, `@APIResponse`, `@Operation`, and `@Parameter` are used by the
MicroProfile OpenAPI and provides more information on the user interface.
If you want to learn more about these annotations and how they are used see
https://openliberty.io/guides/microprofile-openapi.html[Documenting RESTful APIs]
* `@Context` injects this HttpServletRequest into the method parameter.
* `@PathParam` injects a custom `item` and `price` from the GET request into the
method parameter.
...
sessions will become invalid and the data no longer accessible.  This only applies
to the session that called the method.
...
The `sessionCache` feature is most valuable when it is connected to at least
one other member, but can also be used on a single server for failover.
There are two different ways session caching can behave in a server environment:
...
that are also running with the `sessionCache` feature and configured to be part
of the same cluster.

=== What is JCache
JCache stands for Java Caching and is an interface to standardize caching on the
Java platform. The `sessionCache` feature makes use of JCache which allows
for session persistence by providing a common cache of session data between servers.

The `sessionCache` feature does not include a JCache implementation.
...
First, include the `sessionCache` feature in the server.xml file.
...
endpoint is on the server at port 8090, and the `getFromCart` endpoint is on the
server at port 8091.
...
It may take some time before build is complete. If the tests pass, you will see
...
To see whether the tests detect a failure, change one of the expected strings
from either of the match tests.  When the `getFromCart` endpoint is reached the
expected value and actual value will no longer match and an exception will be thrown.
