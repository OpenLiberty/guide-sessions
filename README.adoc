...
The application that you’ll be working with is a shopping cart web service that uses JAX-RS, which is a Java API for building RESTful web services.
...
endpoint on the UI, clicking the `Try it out` button, providing an item and a price, 
and then clicking the `Execute` button.
...
endpoint on the UI, clicking the `Try it out` button, 
and then clicking the `Execute` button. The GET request
...
Then, make a GET request to `/cart` by expanding the GET
endpoint on the UI, clicking the `Try it out` button, and then clicking the `Execute` button.
