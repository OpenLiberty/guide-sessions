...
:page-seo-title: Caching and persisting session data
...
theory, and create a microservice that will persist a user's shopping cart data.
You will run this microservice on two servers accessing the same session cache.
...
information, are maintained on a clients computer. For security reasons, using 
`sessions` is preferred over using `cookies` for sensitive data. Sessions hide data 
from users. Cookies store data on the clients computer, and can be manipulated by a 
