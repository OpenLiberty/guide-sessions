...
First, make a POST request to `/cart/{item}&{price}` by expanding POST endpoint on the UI and clicking on `Try it out` button.
...
https://docs.hazelcast.org/docs/latest/manual/html-single/[Hazelcast Documentation^].
...
As a last step, verify that Hazelcast Cluster is already formed by checking OpenLiberty log located at
`target/liberty/wlp/usr/servers/server1/logs/messages.log`. If you don't see this log, you might need to check 
if multicast is enabled or any firewall is blocking multicast packages.
