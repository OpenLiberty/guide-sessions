...
Run the following commands to deploy the applications into 3 pods as defined in kubernetes.yaml:
[role='command']
```
...
Pause the pod for the GET request by the following command after replace the `[pod-name]`
[role='command']
```
...
but the `pod-name` is different because the session data is sharing among those pods.
